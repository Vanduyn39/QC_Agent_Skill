...
# [MỚI] Tự động trỏ ra thư mục gốc
...
# Cập nhật đường dẫn tuyệt đối cho file credentials
...
def sync_data_from_sheets(project_name: str):
    try:
        ...
        if data:
            ...
            # Đường dẫn tuyệt đối ra ngoài gốc
            file_path = os.path.join(BASE_DIR, "assets", project_name, "outputs", "bug_tracking_db.json")
            
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            
            df_sheets.to_json(file_path, orient='records', force_ascii=False, indent=4)
            print(f"[Sync] Đã đồng bộ dữ liệu mới nhất từ tab 'Bugs' Google Sheets (lưu JSON) cho dự án {project_name}.")
            return True
...
def get_project_report(project_name: str, mode: str = "daily", start_date: str = None, end_date: str = None):
    ...
    # Đường dẫn tuyệt đối ra ngoài gốc
    file_path = os.path.join(BASE_DIR, "assets", project_name, "outputs", "bug_tracking_db.json")
    output_dir = os.path.join(BASE_DIR, "assets", project_name, "outputs")
    os.makedirs(output_dir, exist_ok=True)

    if not os.path.exists(file_path):
        return json.dumps({"error": f"Không tìm thấy file tracking lỗi (JSON) cho {project_name}."})
